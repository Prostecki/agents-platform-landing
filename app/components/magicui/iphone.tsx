...
export function Iphone({ src, videoSrc, children, className, style, ...props }: IphoneProps) {
  ...
  const screenClip: React.CSSProperties = {
    position: "absolute",
    left: `${LEFT_PCT}%`,
    top: `${TOP_PCT}%`,
    width: `${WIDTH_PCT}%`,
  ...
  return (
    <div
      ...
      {...props}
    >
      {/* Arbitrary JSX children rendered inside the screen area */}
      {children && (
        <div style={screenClip}>
          {children}
        </div>
      )}
      ...
      {/* Video overlay */}
      {hasVideo && (
        <div className="pointer-events-none" style={screenClip}>
          <video
      ...
      {/* SVG phone frame — rendered on top, screen punched out when media/children present */}
      <svg
      ...
      >
        ...
        {/* Screen background (visible when no media; masked out when media present) */}
        <path
